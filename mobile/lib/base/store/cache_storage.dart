...
import 'package:universal_html/html.dart'if (kkIsWeb) "dart:html" as html_cache;
import 'package:wordzoo/utils/logger.dart';

class Storage {
  ///constant name key
  static const listOfPassEntity = 'listOfPassEntity';
  static const pointForFreeEntity = 'pointForFreeEntity';
...
class SharedPreferencesStorage {
  ...
  Future<void> saveMapByStringKey({required Map<String, dynamic> map, required String key}) async {
    await prefs?.setString(key, jsonEncode(map));
  }
  Map<String, dynamic> getMapByKey({required String key}){
    Map<String, dynamic> map ={};
    try{
      map =  jsonDecode(prefs?.getString(key)??'') as Map<String, dynamic>;
    }
    catch(e){
      map ={};
    }
  ...
  Future<Map<String, String>> getStringByKeys(List<String> keys) async {
    ...
    for (String key in keys) {
      keyLogoutSessions[key] = await getString(key);
    }
    return keyLogoutSessions;
  }
  ...
  Future<bool> isExitDynamicStringKey(String key) async {
    return  prefs!=null && prefs!.containsKey(key);
  }
...
class LocalStorage {
  ...
  int getInt(String key,{int? valueDefault}) {
    return( double.tryParse(html_cache.window.localStorage[key] ?? "${valueDefault??0}")??0).toInt();
  }
  ...
  int getDynamicInt(String dynamicKey) {
    return (double.tryParse(html_cache.window.localStorage[dynamicKey] ?? '0')??0).toInt();
  }
...
class SessionStorage {
  ...
  int getInt(String key) {
    return (double.tryParse(html_cache.window.sessionStorage[key] ?? '0')??0).toInt();
  }
...
class CookieStorage {
  ...
  String getString(String key) {
    ...
    return (cookieData.containsKey(key) ? cookieData[key] : '').toString();
  }

  int getInt(String key) {
    Map<dynamic, dynamic> cookieData = convertToMap();
    return (double.tryParse(cookieData.containsKey(key) ? (cookieData[key]).toString() : '0')??0).toInt();
  }
