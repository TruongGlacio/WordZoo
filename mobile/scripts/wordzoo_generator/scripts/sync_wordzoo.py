...
def update_is_image_by_name(data: dict, wordzoo_dir: Path):
    """
    Check whether category/subcategory/entity image exists.

    If image exists:
        is_image_by_name = False

    If image missing:
        is_image_by_name = True
    """
    ...
    def image_exists(relative_path: str) -> bool:
        ...
        image_file = wordzoo_dir / relative_path
        return image_file.exists()

    for category in data.get("categories", []):

        category["is_image_by_name"] = not image_exists(
            category.get("real_image") or category.get("icon")
        )

        for sub in category.get("subcategories", []):

            sub["is_image_by_name"] = not image_exists(
                sub.get("real_image") or sub.get("icon")
            )

            for entity in sub.get("entities", []):

                entity["is_image_by_name"] = not image_exists(
                    entity.get("real_image")
                )
...
def main():
    ...
    data = update_is_image_by_name(data, wordzoo_dir)
